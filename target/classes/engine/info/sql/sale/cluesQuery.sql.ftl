select *  from (
   ...
   from t_sales_clues t
	...
	where stage<>'potentialbusiness'
	and 
	case when #{data.session.userInfo.roleName}='Administrator' then 1=1
	     when #{data.session.userInfo.roleName}='销售总监'  then 1=1
	else t.seller = #{data.session.userInfo.empId} or t.presales = #{data.session.userInfo.empId} end
	group by t.id
    )a